...
export async function login(req, res, next) {
  try {
    ...
    if(!isPasswordValid) return res.status(401).json({ error: { message: "Invalid credentials" } })

    const token = signToken({userId: existingUser._id, email: existingUser.email, role: existingUser.role})
